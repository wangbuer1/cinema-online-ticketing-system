...
public class MainCtrl extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("login"))
		{
			...
			String utype = request.getParameter("qx");
			String pagerandom = request.getParameter("pagerandom")==null?"":request.getParameter("pagerandom");
			String random = (String)request.getSession().getAttribute("random");
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null)
			{
				request.setAttribute("random", "");
				go("/qtlogin.jsp", request, response);
			}
			else{
				String sql1="";
				if (utype.equals("注册顾客"))
				{
					sql1 = "select * from gukezhuce where yonghuming='"+username+"' and mima='"+password+"' and issh='是'"; 
			...
			if(userlist1.size()==1)
			{
				...
				request.getSession(). setAttribute("qx", utype);  
		...
		if(ac.equals("adminlogin"))
		{
			...
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null)
			{
				...
				if (utype.equals("管理员"))
				{
				 sql1 = "select * from prousers where username='"+username+"' and pwd='"+password+"'  and qx='管理员'"; 
				 }
				else
				{
					sql1 = "select * from prousers where username='"+username+"' and pwd='"+password+"'  and qx='工作人员'"; 
				}
			...
			if(userlist1.size()==1)
			{
				...
				if (utype.equals("普通用户"))
				{
				request.getSession(). setAttribute("qx",  utype); 
				}
				else
				{ 
				request.getSession(). setAttribute("qx",  userlist1.get(0).get("qx"));
				}
				gor("backindex.jsp", request, response);
			}else{
				request.setAttribute("error", "");
				go("/login.jsp", request, response);
			}
		...
		if(ac.equals("uppass"))
		{
			...
			HashMap m = dao.getmaps("yonghuming",(String)request.getSession().getAttribute("username"), "gukezhuce");
			...
			String sql = "update gukezhuce set mima='"+userpass+"' where yonghuming='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
		...
		if(ac.equals("adminuppass"))
		{
			...
			HashMap m = dao.getmaps("username",(String)request.getSession().getAttribute("username"), "prousers");
			...
			//String id = (String)user.get("id");
			String sql = "update prousers set pwd='"+userpass+"' where username='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
			request.setAttribute("suc", "");
			go("mod.jsp", request, response);
			}
